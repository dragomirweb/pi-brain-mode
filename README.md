...
`/brain gate <cmd|auto|off>` sets the persisted quality-gate command. Auto mode checks the root package first, then discovers packages from the delegation's read paths and reported changed files. It respects npm, pnpm, Yarn, and Bun, choosing the first available verification script in this order: `check`, `verify`, `test:ci`, `typecheck`, `tsc`, `test`. Multiple affected workspace packages are gated together. If nothing is found, each delegation result carries an explicit "Quality gate: none configured or discovered" notice. An explicit command is still useful when the repository needs a custom cross-workspace gate; it is handed to the worker and its result to the reviewer.
...
When the reviewer is enabled, the orchestrator gains a `delegate_to_reviewer` tool. The reviewer inspects the coder's diff, verifies the quality gate (it receives the gate result the extension already ran and spot-checks rather than blindly re-running), and runs the optional Fallow check as `git diff --no-ext-diff | fallow audit --diff-stdin` when available. A missing or failed optional diagnostic is reported but cannot prevent the final structured verdict. The reviewer judges the change against the stated `intent`/`acceptanceCriteria` and returns pass/warn/fail plus findings. It is read-only: every fix goes back through the coder and quality gate. `intent` and `reads` default to the most recent delegation, so a bare `delegate_to_reviewer` call reviews the last change.
...
Every delegation (coder, read-only run, review) records its task summary, changed files, explicit outcome (completed/blocked/failed/aborted/timed-out), gate result, review lifecycle (including skipped/error), structured check counts, executing model, RPC run id when available, duration, error summary, and worker/reviewer cost split. The richer lifecycle state prevents a failed attempt from being mistaken for completed work after compaction. The journal is:
...
- `--brain-gate-command <cmd>`: post-delegation quality gate (default: auto-detect a root or affected-workspace verification script; `off` to disable). Also settable per repo at runtime with `/brain gate <cmd|auto|off>` (persisted).
...
When [pi-subagents](https://github.com/nicobailon/pi-subagents) >= 0.34 is installed, `delegate_to_coder` and `delegate_to_reviewer` use its documented `subagents:rpc:v1` API. Each call starts a one-step asynchronous chain with the packaged `brain-coder`, `brain-runner`, or `brain-reviewer` definition and a strict role-specific output schema. The extension polls the versioned lifecycle artifact for progress, usage, changed files, errors, and controlled structured output; malformed results are rejected before they can be treated as completed work. If pi-subagents marks a run failed only because an earlier child-tool error remained in its lifecycle state, pi-brain can recover a later schema-valid `structured_output` from the child transcript; output submitted before a later error is rejected as stale, and timeouts/interruptions remain failures. Cancellation uses RPC `stop`. Availability failures fall through to the local process spawner, while genuine task failures remain visible and still run the quality gate because files may have changed.

On pi-subagents 0.34, its detached runner may emit a background failure notification before pi-brain reads and recovers the final result. The delegation result and `/brain log` show the corrected outcome, but an already-emitted upstream notification cannot be retracted by this extension.
